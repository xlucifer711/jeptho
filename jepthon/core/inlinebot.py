...
def main_menu():
    text = f"**- [سـورس كرستين ](https://t.me/cr_source)\
        \n المسـاعد\
        \n\
        \n◽ لسورس كرستين  لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" ᥴ.ᖇ ᥉᥆υᖇᥴᥱ UsᴇʀBoᴛ", "https://t.me/cr_source"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس كرستين 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس كرستين  🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس كرستين  🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/cr_source"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/gro_up_1",
                ),
